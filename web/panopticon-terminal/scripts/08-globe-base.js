class Globe {
  ...
  setData(entities) { this.entities = entities.filter((entity) => validGeo(entity.geo)); this.byId = new Map(this.entities.map((entity) => [entity.id,entity])); this.links = uniqueLinks(this.entities); }
  setLayers(layers) { this.layers = new Set(layers); }
  setView(view) { this.view = view; }
  setSensor(sensor) { this.sensor = sensor; }
  setTimeline(value) { this.timeline = value; }
  select(id) { this.selected = id; this.ping(id); }
  ping(id) { this.pings.set(id, performance.now()); }
  resize() { const rect = this.canvas.getBoundingClientRect(); const dpr = Math.min(devicePixelRatio || 1, 2); this.canvas.width = Math.max(1, Math.round(rect.width*dpr)); this.canvas.height = Math.max(1, Math.round(rect.height*dpr)); this.ctx.setTransform(dpr,0,0,dpr,0,0); this.width=rect.width; this.height=rect.height; }
  ...
  frame(now) {
    ...
    this.draw(now); this.onCamera?.({lat:this.lat,lon:normalizeLon(this.lon),altitude:7600/Math.max(.7,this.zoom)}); requestAnimationFrame(this.frame);
  }
}
